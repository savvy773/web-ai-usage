...
- Gemini: refresh가 조금 늦어져도 안전하게 `resetAt` 또는 `remainingText`가 있는 모델 row가 3개 이상일 때만 usage 출력으로 인정합니다.
...
Gemini는 `/model` 화면이 여러 번 redraw될 수 있으므로, 처음 발견된 `Model usage` 섹션이 아니라 퍼센트 값이 들어 있는 마지막 렌더링 섹션을 우선 사용합니다. 패널 안에서는 `label + bar + percent + Resets` row 구조를 신뢰해 새 모델명도 받을 수 있습니다. 패널 밖 fallback은 `Flash`, `Flash Lite`, `Pro`, `gemini-*`처럼 좁은 label만 인정하고 `37% used`, `left`, `limit`, `quota`처럼 하단 상태줄/잔여량 문맥에 붙은 percent는 Gemini model usage로 쓰지 않습니다.
...
2. bar row parser가 reset 정보가 있는 모델 row 3개 이상을 회수하면 우선 사용합니다.
3. 부족하면 fallback 후보 line을 만듭니다.
   - 정규화된 line 배열
   - raw output에서 `│┃║`를 줄바꿈으로 바꾼 line
   - raw output 전체를 한 줄로 붙인 line
4. 다음 parser들을 실행합니다.
   - bar row parser: bar가 포함된 row를 결합하고 label/percent/reset을 파싱. `Model usage` 패널 안에서는 구조가 명확하면 새 모델명도 label로 인정합니다.
...
5. label을 clean합니다.
   ...
   - percent 이후 text 제거
   - `Resets:` 이후 text 제거
   - 패널 밖 fallback에서는 `Flash`, `Flash Lite`, `Pro`, `gemini-*` 외 label은 저장하지 않음
...
7. 최종적으로 reset 정보가 있는 model usage label이 3개 이상이면 Gemini provider는 `ok`가 될 수 있습니다.
...
- Browser reload/F5: SSR payload를 먼저 렌더링한 뒤 `/api/usage/refresh`를 호출해 새 수집을 시작합니다. reload는 사용자가 명시적으로 새로고침한 동작이라 화면 cooldown이 남아 있어도 refresh 요청을 시작합니다.
...
- Gemini `markers=quota-percent`만 있고 `model-screen`이 없으면 parser가 모델 row를 놓친 것이 아니라 `/model` 화면이 아직 뜨지 않은 상태입니다.
- Gemini `parsed-models=1/3`: parser가 인정한 모델 row 수입니다. Gemini provider는 reset 정보가 있는 모델 usage가 3개 이상이어야 `ok`가 될 수 있습니다.
